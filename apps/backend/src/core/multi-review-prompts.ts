...
const STALE_MULTI_REVIEW_WORKTREE_WORDING: WorktreeSnapshotWording = {
  clean:
    "**Pinned worktree state (known stale)**: the backend confirmed this environment worktree was clean when the review started, but later detected that the worktree changed. This recorded state is no longer authoritative. Reconcile the committed range and live worktree before deciding the current change scope, and record any scope you cannot verify as a limitation.",
  dirty:
    "**Pinned worktree state (known stale)**: the backend observed these uncommitted paths when the review started, but later detected that the worktree changed. This recorded path set is no longer authoritative. Reconcile it against the live worktree, include newly observed change paths in the review, and record any scope you cannot verify as a limitation.",
};
...
export function createMultiReviewerPrompt(input: {
  ...
  return [
    ...
    worktreeSnapshotSection(
      input.worktree ?? UNPROBED_WORKTREE,
      input.worktreeChangedDuringReview
        ? STALE_MULTI_REVIEW_WORKTREE_WORDING
        : MULTI_REVIEW_WORKTREE_WORDING,
    ),
    buildReviewBody({
...
function scopeReconciliationRule(
  ...
  if (worktreeChangedDuringReview) {
    rules.push(
      "The repository worktree changed after this Multi Review started. The review deliberately continued instead of discarding completed work. Preserve that fact as a limitation because reviewer reports may reflect different observed worktree states. A report may therefore cover a scope that excludes changes added during the review: do not carry its clean findings, passing validation, or ready verdict into the consolidated result.",
    );
  }
