...
describe("waitFor", () => {
  ...
  test("times out when ConnectionRefused never recovers and names the code", async () => {
    ...
    ).rejects.toThrow("Timed out waiting for ACP state: undefined (last error: ConnectionRefused)");
...
describe("Cursor todo list helpers", () => {
  ...
  test("merges by id, appends new items, and replaces when merge is false", () => {
    ...
    expect(
      mergeCursorTodos(current, [{ id: "9", content: "Only", status: "pending" }], false),
    ).toEqual([{ id: "9", content: "Only", status: "pending" }]);
  });
...
function spawnTaskPart(overrides: Partial<BridgeToolPart> = {}): BridgeToolPart {
  return {
    type: "tool-invocation",
    content: "Task",
