...
describe("NativeAgentService", () => {
      ...
      async ({ service }) => {
        ...
        expect(
          service
            ...
            .every(
              (entry) => entry.eventualOutcome === "expired" && entry.providerState === "idle",
            ),
        ).toBe(true);
      },
    );
  ...
  test("starts and stops the observe-only timer with the service lifecycle", async () => {
    ...
    await withService(
      ...
      async ({ service }) => {
        ...
        await waitForCondition(() => listPendingInteractions.mock.calls.length > afterInit);
  ...
  describe("OpenCode incomplete-turn recovery", () => {
      ...
      await withService(
        ...
        async ({ storage, service }) => {
          ...
          expect((await storage.getNativeAgentSession(key))?.dispatchedRequestIds).toContain(
            openCodeIncompleteTurnRequestId("assistant-1"),
          );
    ...
    test("rejects a manual prompt during the final automatic dispatch handoff", async () => {
      ...
      const { provider, send } = createProviderStub("opencode", {
        activity: async () => "idle",
        messages: async () => [stalledUser, stalledAssistant("assistant-final-handoff")],
      });
      ...
      await withService(
        ...
        async ({ storage, service }) => {
          ...
          await captureWarnings(async () => {
            ...
            await waitForCondition(() => send.mock.calls.length === 1);
            await expect(
              service.claimOpenCodeManualPrompt({
                environmentId: "env-1",
                logicalSessionKey: "tab-1",
                providerSessionId: "provider-1",
    ...
    test("does not continue unattended sessions or turns with queued prompts", async () => {
      ...
      await withService(
        ...
        async ({ storage, service }) => {
          const pipelineKey = nativeAgentSessionStorageKey("env-1", "opencode", "pipeline-tab");
          await storage.adoptNativeAgentSession({
            key: pipelineKey,
            environmentId: "env-1",
            agent: "opencode",
          ...
          const queuedKey = nativeAgentSessionStorageKey("env-1", "opencode", "queued-tab");
          await storage.adoptNativeAgentSession({
            key: queuedKey,
            environmentId: "env-1",
            agent: "opencode",
  ...
  test("bounds concurrent completion notification delivery", async () => {
    const environmentIds = Array.from({ length: 12 }, (_unused, index) => `env-${index + 1}`);
    let activityState: ProviderActivityState = "working";
    const { provider } = createProviderStub("codex", {
      activity: async () => activityState,
    });
  ...
  describe("startup launch reconciliation", () => {
    ...
    ])("dispatches an attachment-only startup prompt for %s too", async (agent, modes) => {
    ...
    test("drains an in-flight launch before shutdown resolves", async () => {
      ...
      const dataDir = await fs.mkdtemp(path.join(tmpdir(), "orkestrator-native-shutdown-drain-"));
      const storage = await createStorage(dataDir);
      await addEnvironment(storage, {
        pendingAgentLaunch: true,
        defaultAgent: "codex",
    ...
    test("retries a transiently failed startup launch after a delay", async () => {
      ...
      await withService(
        ...
        async ({ storage, service }) => {
          ...
          expect(internals(service).launchRetryAt.get("env-1")).toBeGreaterThanOrEqual(
            before + 10_000,
          );
        },
      );
    ...
    test("disposes providers whose environment has gone away", async () => {
      ...
      const dataDir = await fs.mkdtemp(path.join(tmpdir(), "orkestrator-native-prune-"));
      const storage = await createStorage(dataDir);
      await addEnvironment(storage);
      await storage.addEnvironment({
      ...
      try {
        ...
        expect([...internals(service).providers.keys()]).toEqual(["env-1\u0000codex"]);
