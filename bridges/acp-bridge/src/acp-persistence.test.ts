...
describe("ACP bridge", () => {
  ...
  test("drops a persisted model id that is blank, oversized, or not a string", async () => {
    ...
    expect(session.messages.slice(1).map((message) => "modelId" in message)).toEqual([
      false,
      false,
      false,
      false,
  ...
  test("restores nested child parentTaskUseId after a bridge restart", async () => {
    ...
    await waitFor(
      () =>
        fs
          .readFile(resolve(stateDirectory, "state.json"), "utf8")
    ...
    expect(restoredParts.find((part) => part.toolUseId === "cursor-child-grep-1")).toMatchObject({
      parentTaskUseId: "cursor-subagent-1",
    });
    expect(restoredParts.find((part) => part.toolUseId === "cursor-child-claude-5")).toMatchObject({
      parentTaskUseId: "cursor-subagent-1",
    });
    expect(
      restoredParts.find((part) => part.toolUseId === "cursor-child-self-7"),
    ...
    expect(restoredParts.find((part) => part.toolUseId === "cursor-subagent-1")).toMatchObject({
      toolName: "task",
    });
  });
  ...
  test("bounds replay metadata before publishing and persists the trimmed transcript", async () => {
    ...
    expect(Buffer.byteLength(JSON.stringify(session.messages))).toBeLessThanOrEqual(
    ...
    const retainedTool = session.messages
      .flatMap((message) => message.parts)
      .find((part) => part.toolTitle === "Read huge-c.json");
    expect(retainedTool?.toolUseId).toBe("live-huge-c");
    expect((retainedTool?.toolArgs as { payload?: string } | undefined)?.payload?.length).toBe(
      480 * 1024,
    );
    expect(
      session.messages
        .flatMap((message) => message.parts)
        .find((part) => part.toolUseId === "live-huge-b"),
    ...
    const persisted = await waitFor(
      ...
      (value) =>
        value.sessions[0]?.messages
          .flatMap((message) => message.parts)
          .some((part) => part.toolTitle === "Read huge-c.json") === true,
    );
    expect(persisted.sessions[0]?.status).toBe("idle");
    expect(Buffer.byteLength(JSON.stringify(persisted.sessions[0]?.messages))).toBeLessThanOrEqual(
      maximumTranscriptBytes,
    );
  ...
  test("bounds all persisted sessions while retaining structured output across restart", async () => {
    ...
    await waitFor(
      async () =>
        nativeFetch(`${first.base}/session/${largeTranscript.id}/status`, {
          headers: first.headers,
    ...
    await waitFor(
      async () =>
        nativeFetch(`${first.base}/session/${structuredSession.id}/status`, {
          headers: first.headers,
        }).then((response) => response.json()) as Promise<{ status: string }>,
      (value) => value.status === "idle",
    );
  ...
  test("reports a turn lost to a bridge restart as unknown, not dispatched", async () => {
    ...
    expect(
      await nativeFetch(`${second.base}/session/${created.id}/dispatch?requestId=in-flight-1`, {
        headers: second.headers,
  ...
  test("restores a normalized composer across a bridge restart", async () => {
    ...
    expect((await fs.readdir(stateDirectory)).some((entry) => entry.includes("corrupt"))).toBe(
      false,
    );
  });
  ...
  test("resets one malformed composer without discarding its sibling sessions", async () => {
    ...
    expect(
      ((await survivor.json()) as { composer: { models: unknown[] } }).composer.models,
    ).toHaveLength(2);
