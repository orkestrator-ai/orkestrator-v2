/**
 * The attachment read is a trust boundary, so it is tested as one.
 *
 * `prompt-attachments.ts` is a deliberate copy of the ACP bridge's reader
 * rather than a shared import — three independent implementations of one
 * reviewed boundary, not one implementation with three callers that a refactor
 * could quietly weaken for all of them. That argument only holds while each
 * copy is independently tested, which is what this file is for: the path
 * arrives over HTTP, so it must stay inside the workspace, refuse a symlinked
 * component, stay bounded, and be re-checked after the read.
 */
...
async function workspace(): Promise<string> {
  const directory = await fs.mkdtemp(resolve(os.tmpdir(), "cursor-attachments-"));
