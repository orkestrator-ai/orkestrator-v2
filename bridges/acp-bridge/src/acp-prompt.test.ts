...
describe("structuredPromptInstruction", () => {
  ...
  test("permits prose progress updates but not drafted JSON", () => {
    ...
    expect(instruction).toContain("never send a JSON object or array as a progress update");
...
describe("ACP bridge", () => {
  test("classifies resource_exhausted and unavailable provider errors as retriable", () => {
    ...
    expect(RETRIABLE_PROVIDER_ERROR.test("RetriableError: [unavailable] PING timed out")).toBe(
      true,
    );
    expect(
      RETRIABLE_PROVIDER_ERROR.test("GoogleGenerativeAIFetchError: [unavailable] PING timed out"),
    ).toBe(true);
    expect(RETRIABLE_PROVIDER_ERROR.test("permission denied: [invalid_argument] Error")).toBe(
      false,
    );
    expect(retriableProviderError(new Error("RetriableError: [unavailable] PING timed out"))).toBe(
      true,
    );
    expect(retriableProviderError(new Error("cursor ACP session/prompt timed out"))).toBe(false);

    expect(
      FLATTENED_RETRIABLE_PROVIDER_SUFFIX.test(
    ...
    expect(
      FLATTENED_RETRIABLE_PROVIDER_SUFFIX.test("Error: RetriableError: [invalid_argument] Error"),
    ).toBe(false);
    expect(
      FLATTENED_RETRIABLE_PROVIDER_SUFFIX.test(
        "Error: RetriableError: [unavailable] PING timed out\n\nMore assistant text",
  ...
  test("drives an ACP session and rehydrates a parked permission", async () => {
    ...
    const createdResponse = await nativeFetch(`${base}/session/create`, {
      method: "POST",
      headers,
    });
    ...
    const approval = await waitFor(
      async () =>
        nativeFetch(`${base}/session/${created.id}/approvals`, { headers }).then((response) =>
          ...
          approvals: Array<{ id: string; approvalId: string; title: string; kind: string }>;
        }>,
    ...
    const session = await waitFor(
      async () =>
        nativeFetch(`${base}/session/${created.id}`, { headers }).then((response) =>
          response.json(),
        ) as Promise<{ status: string; messages: Array<{ content: string; parts: unknown[] }> }>,
      (value) => value.status === "idle",
    );
  ...
  test("deduplicates session creation and prompt dispatch by durable keys", async () => {
    ...
    const first = await spawnBridge({
      stateDirectory,
      env: { FAKE_ACP_COUNTER_FILE: counterFile },
    });
    const create = () =>
      nativeFetch(`${first.base}/session/create`, {
        method: "POST",
        headers: first.headers,
    ...
    const send = () =>
      nativeFetch(`${first.base}/session/${created.id}/prompt`, {
        method: "POST",
        headers: first.headers,
    ...
    const second = await spawnBridge({
      stateDirectory,
      env: { FAKE_ACP_COUNTER_FILE: counterFile },
    });
  ...
  test("answers whether a request id was ever dispatched", async () => {
    ...
    const dispatchStatus = (requestId: string) =>
      nativeFetch(
    ...
    expect(
      await nativeFetch(`${bridge.base}/session/${created.id}/dispatch`, {
        headers: bridge.headers,
  ...
  test("attaches the agent process without dispatching a turn", async () => {
    ...
    const attach = () =>
      nativeFetch(`${second.base}/session/${created.id}/attach`, {
        method: "POST",
        headers: second.headers,
        body: "{}",
      });
    ...
    await expect(
      nativeFetch(`${second.base}/session/${created.id}`, {
        headers: second.headers,
      }).then((response) => response.json()),
  ...
  test("keeps later attaches and prompts behind an in-flight session load", async () => {
    ...
    const settlesWithin = (pending: Promise<Response>, milliseconds: number) =>
      Promise.race([
        pending.then(
          () => true,
          () => true,
  ...
  test("rejects a second composer patch that races one already applying", async () => {
    ...
    const config = (await nativeFetch(`${base}/session/${created.id}/config`, { headers }).then(
      (response) => response.json(),
    )) as { selectedModelId?: string };
  ...
  test("uses only the current turn when parsing structured output", async () => {
    ...
    const result = (await nativeFetch(
      ...
      structuredOutput: { ok: boolean; error?: { code?: string } };
    };
  ...
  test("denies pending permission when cancelled", async () => {
    ...
    const approvals = (await nativeFetch(`${base}/session/${created.id}/approvals`, {
      headers,
  ...
  test("keeps live approvals when a superseded agent process exits", async () => {
    ...
    const parked = await waitFor(
      async () =>
        nativeFetch(`${base}/session/${created.id}/approvals`, { headers }).then((response) =>
          response.json(),
        ) as Promise<{ approvals: Array<{ id: string; expiresAt: number; requestedAt: number }> }>,
      (value) => value.approvals.length === 1,
    );
    ...
    await Bun.sleep(100);
    const stillParked = (await nativeFetch(`${base}/session/${created.id}/approvals`, {
      headers,
    }).then((response) => response.json())) as { approvals: unknown[] };
    expect(stillParked.approvals).toHaveLength(1);
    const resolved = await nativeFetch(`${base}/session/${created.id}/approvals/${approval.id}`, {
      method: "POST",
      headers,
  ...
  test("drops approvals when the agent that parked them dies", async () => {
    ...
    const { base, headers } = await spawnBridge({
      env: { FAKE_ACP_LIFECYCLE_FILE: lifecycleFile },
    });
    const created = (await nativeFetch(`${base}/session/create`, { method: "POST", headers }).then(
      (response) => response.json(),
    )) as { id: string };
    ...
    const status = (await nativeFetch(`${base}/session/${created.id}/status`, { headers }).then(
      (response) => response.json(),
    )) as { status: string };
  ...
  test("dispatches an image-only prompt and indexes every attachment part", async () => {
    ...
    expect(user?.parts.map((part) => part.sourcePartId)).toEqual([
      `${messageId}:1`,
      `${messageId}:2`,
    ]);
  });
