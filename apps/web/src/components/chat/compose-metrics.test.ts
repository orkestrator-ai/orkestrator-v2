...
const COMPOSE_BARS = ["chat/NativeComposeBar.tsx"];
...
describe("compose metrics", () => {
  test("derives a 256px cap from the shared line height", () => {
    ...
    expect(COMPOSE_MAX_INPUT_HEIGHT).toBe(COMPOSE_MAX_LINES * COMPOSE_LINE_HEIGHT + 16);
    expect(COMPOSE_MIN_INPUT_HEIGHT).toBe(28);
    expect(COMPOSE_MIN_INPUT_HEIGHT).toBeLessThan(COMPOSE_MAX_INPUT_HEIGHT);
  });

  test.each(COMPOSE_BARS)("%s sizes its input from the shared constants", (relativePath) => {
    /**
