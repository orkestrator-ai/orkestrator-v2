...
async function invoke<T>(browserUrl: string, authFile: string, command: string, args: Record<string, unknown> = {}): Promise<T> {
...
export async function seedFixture(options: {
  ...
  let project = projects.find((entry) => (entry as Project & { localPath?: string }).localPath === projectPath);
  if (!project) {
  ...
  for (const kind of options.environments) {
    const environment = await invoke<Environment>(options.browserUrl, options.authFile, "create_environment", {
