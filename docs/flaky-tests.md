...
## `backend command I/O coverage > rejects malformed container status framing and invalid encoded sections` (`tests/unit/electron/commands-registry-terminal.test.ts:1418`)
...
- **Failure:** `expect(received).toThrow(expected)` with `Expected substring: "Malformed"`; the received message was `Command failed: docker exec container-1 bash -lc ...`, and the log also recorded `this test timed out after 5000ms` (duration: 5,013.59 ms).
- **Suite counts:** 3,726 passed, 1 skipped, 2 failed; 16,602 expect() calls, 238.8 s.
- **Isolated rerun:** `bun test tests/unit/electron/commands-registry-terminal.test.ts` → passed in 23.0 s.
- **Follow-up:** A later `--parallel=4` run of the same command on a quieter host passed in 79.2 s without this failure.
- **Hypothesis:** The case asserts on a rejection produced by a real `docker exec` fake within Bun's 5-second default budget. Under host contention the fake process did not return before the outer timeout, so the timeout message replaced the expected `Malformed` rejection. A recurrence should give the case an explicit budget rather than loosening the `Malformed` assertion, which is the actual product behaviour under test.
