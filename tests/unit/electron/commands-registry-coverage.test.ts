...
describe("direct backend command registry coverage", () => {
  ...
  test("stops each bridge, reports authenticated OpenCode health, and delegates model caching", async () => {
    ...
    const cacheAgentModelCatalog = mock(async (_agent: string, _models: unknown[]) => cachedAgents);
    const context = contextWithStorage({
    ...
    await expect(invoke("get_agent_model_catalog_cache", {}, context)).resolves.toEqual(
      cachedAgents,
    );
    await expect(
      invoke(
  ...
  test("validates and delegates host agent model catalogue writes", async () => {
    ...
    await expect(
      invoke("cache_agent_model_catalog", { agent: "claude", models: claudeModels }, context),
    ).resolves.toEqual(cachedAgents);
    ...
    await expect(
      invoke("cache_agent_model_catalog", { agent: "codex", models: [] }, context),
  ...
  test("launches validated browser URLs and returns both DNS resolution outcomes", async () => {
    ...
    await expect(invoke("validate_domains", { domains: ["localhost"] }, context)).resolves.toEqual([
      expect.objectContaining({
        domain: "localhost",
        resolvable: true,
      }),
    ...
    const launcher =
      process.platform === "darwin"
        ...
        : process.platform === "win32"
          ? "explorer.exe"
          : "xdg-open";
    expect(await commandLogContents()).toContain(`${launcher} https://example.com/a?x=1&y=2`);
  });
  ...
  test("forwards storage reads and deletes with validated identifiers and revisions", async () => {
    ...
    const context = contextWithStorage({
      ...
      transferPromptQueueMessageToComposeDraft: value("transferPromptQueueMessageToComposeDraft", {
        removed: { id: "m1" },
      }),
    ...
    const cases: Array<{
      ...
      {
        command: "load_session_buffer",
        args: { sessionId: "session-1" },
        expected: "terminal output",
        call: { method: "loadSessionBuffer", args: ["session-1"] },
      },
      {
        command: "get_pane_layout",
        args: { environmentId: "env-1" },
        expected: { revision: 3 },
        call: { method: "getPaneLayout", args: ["env-1"] },
      },
      {
        command: "delete_pane_layout",
        args: { environmentId: "env-1" },
        expected: undefined,
        call: { method: "deletePaneLayout", args: ["env-1"] },
      },
      {
        command: "get_looped_review_workflow",
      ...
      {
        command: "get_build_pipeline",
        args: { pipelineId: "pipeline-1" },
        expected: { id: "pipeline-1" },
        call: { method: "getBuildPipeline", args: ["pipeline-1"] },
      },
      {
        command: "list_build_pipelines",
        args: { projectId: "project-1" },
        expected: [{ id: "pipeline-1" }],
        call: { method: "listBuildPipelines", args: ["project-1"] },
      },
      {
        command: "delete_build_pipeline",
        args: { pipelineId: "pipeline-1" },
        expected: undefined,
        call: { method: "deleteBuildPipeline", args: ["pipeline-1"] },
      },
      {
        command: "get_prompt_queue",
        args: { queueKey: "queue-1" },
        expected: { queueKey: "queue-1" },
        call: { method: "getPromptQueue", args: ["queue-1"] },
      },
      {
        command: "list_prompt_queues",
        args: { environmentId: "env-1" },
        expected: [{ queueKey: "queue-1" }],
        call: { method: "listPromptQueues", args: ["env-1"] },
      },
      {
        command: "enqueue_prompt_queue_message",
        args: { queueKey: "queue-1", environmentId: "env-1", message: { id: "m1" } },
        expected: { queueKey: "queue-1" },
        call: { method: "enqueuePromptQueueMessage", args: ["queue-1", "env-1", { id: "m1" }] },
      },
      {
        command: "requeue_prompt_queue_message",
        args: { queueKey: "queue-1", environmentId: "env-1", message: { id: "m1" } },
        expected: { queueKey: "queue-1" },
        call: { method: "requeuePromptQueueMessage", args: ["queue-1", "env-1", { id: "m1" }] },
      },
      {
        command: "remove_prompt_queue_message",
        args: { queueKey: "queue-1", environmentId: "env-1", messageId: "m1" },
        expected: { removed: { id: "m1" } },
        call: { method: "removePromptQueueMessage", args: ["queue-1", "env-1", "m1"] },
      },
      {
        command: "move_prompt_queue_message",
        args: { queueKey: "queue-1", environmentId: "env-1", messageId: "m1", direction: "up" },
        expected: { queueKey: "queue-1" },
        call: { method: "movePromptQueueMessage", args: ["queue-1", "env-1", "m1", "up"] },
      },
      {
        command: "claim_prompt_queue_head",
        args: { queueKey: "queue-1", environmentId: "env-1", expectedMessageId: "m1" },
        expected: { claimed: { id: "m1" } },
        call: { method: "claimPromptQueueHead", args: ["queue-1", "env-1", "m1"] },
      },
      {
        command: "acknowledge_prompt_queue_claim",
        args: { queueKey: "queue-1", environmentId: "env-1", claimToken: "token-1" },
        expected: { queueKey: "queue-1" },
        call: { method: "acknowledgePromptQueueClaim", args: ["queue-1", "env-1", "token-1"] },
      },
      {
        command: "reject_prompt_queue_claim",
        args: { queueKey: "queue-1", environmentId: "env-1", claimToken: "token-1" },
        expected: { queueKey: "queue-1" },
        call: { method: "rejectPromptQueueClaim", args: ["queue-1", "env-1", "token-1"] },
      },
      {
        ...
        expected: { removed: { id: "m1" } },
        call: {
          method: "transferPromptQueueMessageToComposeDraft",
          args: ["queue-1", "env-1", "m1", "draft-1", "environment", "env-1", 2],
        },
      },
      ...
      // 0: a 0 would assert "no draft exists" against a slot that may hold one.
      {
        ...
        call: {
          method: "transferPromptQueueMessageToComposeDraft",
          args: ["queue-1", "env-1", "m1", "draft-1", "environment", "env-1", undefined],
        },
      },
      {
        command: "get_compose_draft",
        args: { draftKey: "compose-1" },
        expected: { draftKey: "compose-1" },
        call: { method: "getComposeDraft", args: ["compose-1"] },
      },
      {
        command: "list_compose_drafts",
        args: { ownerType: "environment", ownerId: "env-1" },
        expected: [{ draftKey: "compose-1" }],
        call: { method: "listComposeDrafts", args: ["environment", "env-1"] },
      },
      {
        command: "delete_compose_draft",
        args: { draftKey: "compose-1", expectedRevision: 7 },
        expected: undefined,
        call: { method: "deleteComposeDraft", args: ["compose-1", 7] },
      },
      {
        command: "get_file_draft",
        args: { draftKey: "file-1" },
        expected: { draftKey: "file-1" },
        call: { method: "getFileDraft", args: ["file-1"] },
      },
      {
        command: "delete_file_draft",
        args: { draftKey: "file-1" },
        expected: undefined,
        call: { method: "deleteFileDraft", args: ["file-1", undefined] },
      },
      {
        command: "get_agent_handoff",
        args: { handoffId: "handoff-1" },
        expected: { handoffId: "handoff-1" },
        call: { method: "getAgentHandoff", args: ["handoff-1"] },
      },
      {
        command: "delete_agent_handoff",
        args: { handoffId: "handoff-1", environmentId: "env-1" },
        expected: undefined,
        call: { method: "deleteAgentHandoff", args: ["handoff-1", "env-1"] },
      },
      {
        command: "prune_agent_handoffs",
        args: { environmentId: "env-1", referencedHandoffIds: ["keep-1"] },
        expected: 2,
        call: { method: "pruneAgentHandoffs", args: ["env-1", ["keep-1"]] },
      },
    ];
  ...
  test("deleting a looped review is gated on the stored version, not on whether the snapshot parses", async () => {
    ...
    await expect(
      invoke("delete_looped_review_workflow", { workflowId: "workflow-1" }, withRecord(null)),
    ).resolves.toBeUndefined();
    expect(deleteLoopedReviewWorkflow).toHaveBeenCalledTimes(2);
  });

  test("deleting a multi review delegates its complete teardown to the supervisor", async () => {
    ...
    await expect(
      invoke("delete_multi_review_workflow", { workflowId: "multi-1" }, context),
    ).resolves.toBeUndefined();
    expect(close).toHaveBeenCalledWith("multi-1");
    await expect(
      invoke("delete_multi_review_workflow", { workflowId: " " }, context),
    ).rejects.toThrow("non-blank string");
    await expect(
      invoke("delete_multi_review_workflow", { workflowId: "multi-1" }, contextWithStorage({})),
    ).rejects.toThrow("supervisor is unavailable");
  });
  ...
  test("admits stable container terminal sessions without starting a PTY", async () => {
    ...
    expect(await invoke("get_terminal_session", { sessionId: first.sessionId }, context)).toEqual({
      id: first.sessionId,
      running: false,
      bootstrapped: false,
    });
    expect(
      await invoke("get_terminal_output_snapshot", { sessionId: first.sessionId }, context),
    ).toEqual({
      output: "",
      revision: 0,
      generation: 1,
  ...
  test("reads, reverts, and deletes local worktree files through environment-scoped handlers", async () => {
    ...
    expect(await fs.readFile(path.join(worktree, "src", "app.ts"), "utf8")).toBe(
    ...
    expect(
      await fs.stat(path.join(worktree, "delete-me.txt")).then(
        () => true,
        () => false,
      ),
    ).toBe(false);
  ...
  test("records paused PR watch intent, refreshes safely, and validates modes", async () => {
    ...
    expect(
      (
        (await invoke("get_pr_monitor_state", {}, context)) as {
          entries: Array<Record<string, unknown>>;
        }
      ).entries[0],
    ).toMatchObject({
  ...
  test("delegates feature build claims and preserves storage conflict results", async () => {
    ...
    expect(claimFeaturePlanBuild).toHaveBeenNthCalledWith(1, "feature-1", "task-winner");
    expect(claimFeaturePlanBuild).toHaveBeenNthCalledWith(2, "feature-1", "task-loser");
    await expect(
      invoke(
        "claim_feature_plan_build",
        {
