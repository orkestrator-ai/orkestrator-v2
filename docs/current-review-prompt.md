...
| Item | Location |
|------|----------|
| Prompt generator | `src/prompts/git-workflows.ts` → `createReviewPrompt(targetBranch)` |
| Shared body | `src/prompts/review-shared.ts` → `buildReviewBody(opts)` |
| Export | `src/prompts/index.ts` |
| UI trigger | `src/components/layout/ActionBar.tsx` → `handleReview()` |
| Tests | `src/prompts/git-workflows.test.ts` |

The shared `buildReviewBody` is also consumed by `createBuildReviewPrompt` in `src/prompts/build-pipeline.ts` (build-pipeline review phase). The only behavioural difference is that the action bar variant allows clarifying questions, while the build-pipeline variant tells the agent not to ask any.
...
|---------|----------|--------|
| Build pipeline review phase | `createBuildReviewPrompt()` in `src/prompts/build-pipeline.ts` | Shares the same body via `buildReviewBody()`. Adds ticket title, description, acceptance criteria, comments, images, and project notes. Tells the agent NOT to ask clarifying questions. |
...
1. `buildReviewBody()` in `src/prompts/review-shared.ts` (shared body) — this is where most edits should land.
2. `createReviewPrompt()` in `src/prompts/git-workflows.ts` if the action-bar-specific framing changes.
3. `createBuildReviewPrompt()` in `src/prompts/build-pipeline.ts` if pipeline framing changes.
4. Assertions in `src/prompts/git-workflows.test.ts` and `src/prompts/build-pipeline.test.ts`.
5. This file so it stays in sync with the generated template (regenerate with `bun -e 'import { createReviewPrompt } from "./src/prompts/git-workflows"; console.log(createReviewPrompt("main"));'`).
