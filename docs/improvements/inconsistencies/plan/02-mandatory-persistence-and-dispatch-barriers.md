...
- [x] Keep one write queue per state file. Both streaming publication and
  mandatory barriers must enter it; no direct final write may overlap a queued
  write using the same temporary filename.
- [x] Have `persistBarrier` create an operation that publishes a snapshot
...
- [x] A previously failed best-effort write must not poison all future barriers.
  Conversely, an earlier successful write cannot satisfy a later mutation's
  barrier. Test both orderings explicitly.
- [x] Keep best-effort coalescing, but do not coalesce away a mandatory caller's
...
- [x] Immediately replace the oversized-state early return with an explicit
...
- [x] Close normal write admission before shutdown drain starts. Serialize the
...
- [x] Preserve intentional stateless mode only when no persistent state path is
...
- [x] Keep attachment validation and the synchronous session claim before any
  provider dispatch. Resolve cold attachment and identity changes first.
- [x] Publish the prepared journal entry together with the final provider agent
...
- [x] Re-check ownership after the barrier and immediately before invoking the
  SDK. A close/cancel arriving while disk I/O waits must not be bypassed.
- [x] On pre-dispatch failure, release the claim and restore transient transcript
...
- [x] Keep a completion observer attached as soon as a run exists. Adding a
  post-acceptance write must never cause a live run promise to become unobserved.
- [x] Review duplicate acknowledgements: an in-memory `accepted` entry whose
...
- [x] Use the corrected barrier for prepared steering records before `run.steer`.
- [x] Revalidate that the same active run is still owned after publication.
- [x] Distinguish publication failure before delivery from failure after delivery.
...
- [x] Do not let a second failing barrier in an error handler erase the original
  dispatch classification. Return an actionable, content-free failure while
  maintaining an observed recovery promise.
- [x] Verify the backend maps unknown outcomes to its existing parked request
...
Implemented as (2026-09-26): `bridges/cursor-bridge/src/http-dispatch-durability.test.ts` (the barrier cases were folded in instead of a separate `persistence-barrier.test.ts`), `http-prompt-outcome.test.ts`, `process-restart.test.ts`, `persistence-bounds.test.ts`, and backend `apps/backend/src/core/http-bridge-provider-prompt-outcome.test.ts`.
...
- [x] New focused durability tests pass alone and alongside existing HTTP and
  persistence suites through the logged runner.
- [x] Cursor bridge typecheck passes; backend typecheck/tests pass if error mapping changes.
- [x] Every mandatory caller observes its own failure and no unhandled rejection appears.
- [x] No prompt or steer starts before the required snapshot publication.
- [x] Unknown outcomes still reconcile without automatic duplicate execution.
- [x] Interim aggregate overflow fails explicitly; step 03 is included before
