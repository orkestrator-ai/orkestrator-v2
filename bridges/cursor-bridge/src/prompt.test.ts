...
describe("context occupancy versus turn spend", () => {
  ...
  test("resolves the gauge window from the curated table when the composer predates it", async () => {
    const state = runningSession();
    // A catalogue hydrated before windows were normalized: the entry has no
    // window of its own, but the model id is a confirmed one.
    state.composer.models = [{ platform: "cursor", id: "grok-4-6", label: "Cursor Grok 4.6" }];
    state.composer.selectedModelId = "grok-4-6";
    const run: FollowableRun = {
      async *stream() {
        yield {
  ...
  test("leaves the gauge unbounded for models with no confirmed window", async () => {
    const state = runningSession();
    state.composer.models = [{ platform: "cursor", id: "composer-2", label: "Composer" }];
    state.composer.selectedModelId = "composer-2";
    const run: FollowableRun = {
      async *stream() {
        yield {
          type: "usage",
