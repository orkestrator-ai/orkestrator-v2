...
describe("direct backend command registry coverage", () => {
  ...
  test("deleting a multi review is gated on the stored phase being terminal", async () => {
    ...
    const withRecord = (value: unknown) =>
      contextWithStorage({
    ...
    for (const phase of ["reviewing", "consolidating", "cancelling", "failed"]) {
      await expect(
        invoke(
          "delete_multi_review_workflow",
          { workflowId: "multi-1" },
          withRecord(record(workflow(phase))),
        ),
      ).rejects.toThrow("must be cancelled before deletion");
    }
    ...
    await expect(
      invoke(
        "delete_multi_review_workflow",
        { workflowId: "multi-1" },
        withRecord(record({ phase: "cancelled" })),
      ),
    ).rejects.toThrow("must be cancelled before deletion");
    ...
    await expect(
      invoke(
        "delete_multi_review_workflow",
        { workflowId: "multi-1" },
        withRecord(record(workflow("cancelled"))),
      ),
    ).resolves.toBeUndefined();
    // So does a record that no longer exists at all.
    await expect(
      invoke("delete_multi_review_workflow", { workflowId: "multi-1" }, withRecord(null)),
    ).resolves.toBeUndefined();
    expect(deleteMultiReviewWorkflow).toHaveBeenCalledTimes(2);
    await expect(
      invoke("delete_multi_review_workflow", { workflowId: " " }, withRecord(null)),
