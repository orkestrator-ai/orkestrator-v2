...
describe("buildPipelineStore backend projection", () => {
  ...
  test("accepts a complete backend-owned snapshot", () => {
    ...
    expect(useBuildPipelineStore.getState().buildEnvironmentIds).toEqual(
      new Set(["env-1"]),
    );
  });
  ...
  test("rejects malformed sessions and out-of-range current indexes", () => {
    ...
    expect(isBuildPipeline(pipeline({
  ...
  test("replacePipeline rejects an invalid trust-boundary snapshot", () => {
    expect(() => useBuildPipelineStore.getState().replacePipeline(
      pipeline({ controller: "renderer" as "backend" }),
  ...
  test("ignores an older revision for the same pipeline", () => {
    useBuildPipelineStore.getState().replacePipeline(
      pipeline({ backendRevision: 5, taskTitle: "new" }),
    );
    const mapBefore = useBuildPipelineStore.getState().pipelines;

    useBuildPipelineStore.getState().replacePipeline(
      pipeline({ backendRevision: 4, taskTitle: "stale" }),
    );

    expect(useBuildPipelineStore.getState().pipelines).toBe(mapBefore);
    expect(useBuildPipelineStore.getState().getPipelineById("pipeline-1")?.taskTitle)
      .toBe("new");
  });

  test("accepts an equal revision as an authoritative refresh", () => {
    useBuildPipelineStore.getState().replacePipeline(
      pipeline({ backendRevision: 5, taskTitle: "before" }),
    );
    useBuildPipelineStore.getState().replacePipeline(
      pipeline({ backendRevision: 5, taskTitle: "after" }),
    );
    expect(useBuildPipelineStore.getState().getPipelineById("pipeline-1")?.taskTitle)
      .toBe("after");
  });
  ...
  test("removes one pipeline and no-ops for an unknown id", () => {
    ...
    expect([...useBuildPipelineStore.getState().pipelines.keys()]).toEqual([
      "pipeline-2",
    ]);
    expect(useBuildPipelineStore.getState().buildEnvironmentIds).toEqual(
      new Set(["env-2"]),
  ...
  test("queries GitHub issues by repository identity and active state", () => {
    ...
    const state = useBuildPipelineStore.getState();
    expect(state.getPipelineForGitHubIssue("acme", "widget", 42))
      .toBe(oldComplete);
    expect(state.getPipelineForGitHubIssue("acme", "widget", 42, true))
      .toBeUndefined();
    expect(state.getPipelineForGitHubIssue("acme", "widget", 43, true))
      .toBe(active);
    expect(state.getPipelineForGitHubIssue("other", "widget", 43))
      .toBeUndefined();
  });
});
