...
export function NativeAgentInteractionCard({
  ...
  const canSubmit = interaction.kind === "mcp-form"
      ...
      const resolution: AgentInteractionResolution = {
        ...
        ...(action === "answer" ? {
          answer: {
            ...
            interactionId: interaction.id,
            sessionId: interaction.sessionId,
            answers: interaction.kind === "mcp-form" && mcpQuestion
        ...
        ...(resolutionFeedback?.trim()
          ? { feedback: resolutionFeedback.trim() }
          : {}),
        resolvedAt: Date.now(),
      };
      const outcome = await onResolve(resolution);
      if (outcome.result === "applied" || outcome.result === "stale" || outcome.result === "already-resolved") {
  ...
  const setQuestionAnswer = (questionId: string, secret: boolean, update: (answer: Answer) => Answer) => {
      ...
      {interaction.presentation.url ? (
        externalUrl ? (
          <Button
            ...
            onClick={() => void openInBrowser(externalUrl).catch(() => setError("Could not open the authorization page."))}
          >
            ...
            return (
              <label key={key} className="block space-y-1.5">
                ...
                {definition.type === "boolean" ? (
                  <input aria-label={typeof definition.title === "string" ? definition.title : key} type="checkbox" checked={value === true} disabled={submitting || expired} onChange={(event) => setValue(event.target.checked)} />
                ) : options.length ? (
                  <select aria-label={typeof definition.title === "string" ? definition.title : key} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" value={String(value ?? "")} disabled={submitting || expired} onChange={(event) => setValue(event.target.value)}>
                  ...
                  <Input
                    ...
                    type={sensitive ? "password" : definition.type === "number" || definition.type === "integer" ? "number" : "text"}
                    value={String(value ?? "")}
                    disabled={submitting || expired}
                    onChange={(event) => setValue(coerceNativeFormFieldValue(event.target.value, definition.type))}
                  />
                )}
                {sensitive ? <span className="block text-[11px] text-muted-foreground">Secret input is discarded when you leave this tab.</span> : null}
              </label>
            );
          })}
        </div>
          ...
          <fieldset key={question.id} className="mt-3 space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {question.options.map((option) => {
                ...
                return (
                  <Button key={option.id} type="button" size="sm" variant={selected ? "default" : "outline"} disabled={submitting || expired} aria-pressed={selected} onClick={() => setQuestionAnswer(question.id, question.secret, (current) => ({
                    ...current,
