...
describe("waitUntil", () => {
  test("awaits an asynchronous predicate until it becomes true", async () => {
    ...
    await waitUntil(async () => {
      calls += 1;
      return calls >= 3;
    }, "async predicate never became true", 1_000);
  ...
  test("throws the caller's diagnostic when an async predicate never holds", async () => {
    await expect(waitUntil(
      ...
      25,
    )).rejects.toThrow("the condition under test");
  });
  ...
  test("evaluates a synchronous predicate before honouring an expired deadline", async () => {
    ...
    await expect(waitUntil(() => false, "expired immediately", 0))
      .rejects.toThrow("expired immediately");
  });
});
