...
module has finished evaluating — which every long-lived process here is. The
backend and all five bridges therefore install `installFatalRejectionGuard`
(`packages/protocol/src/fatal-rejections.ts`) at startup, next to the parent
watchdog: it reports the rejection and keeps running.

The guard is a floor, not a licence. Reaching it means something was missed and
its log line is the evidence; code that can handle its own rejections still
must. Two properties are load-bearing:

- **It is inert under `bun test`** (`NODE_ENV=test`), because bridge suites
  import the entrypoints directly and a run that fails on an unhandled rejection
...
consumer of that signal has a rejection handler. The guard catches what you
miss, but a log line every turn is still a bug.
...
### The patched SSE client

`@opencode-ai/sdk` is patched — see `patches/`, wired in through
`patchedDependencies` in the root `package.json`. The generated SSE client calls
`reader.cancel()` from its abort handler with no `.catch`; when a `reader.read()`
is pending the body stream is already errored by the aborted fetch, so that call
returns a rejected promise nobody owns. That is what `OpenCodeProvider.dispose()`
turned into a dead backend.

Re-check the patch on every SDK bump. It still applies as of 1.18.29, the file
is generated by `@hey-api/openapi-ts` rather than hand-written upstream, and
`bun patch` fails loudly when the target no longer matches. `apps/backend` and
`apps/web` pin the same version on purpose, so one patch covers both. An SDK
upgrade is incomplete until all of the following agree on the new version:

- `apps/backend/package.json` and `apps/web/package.json` dependency pins.
- The root `package.json` `patchedDependencies` key and patch path.
- The filename under `patches/` and the matching entry in `bun.lock`.

Regenerate or rebase the patch against the new SDK rather than only renaming
the old file, and confirm both `dist/gen/core/serverSentEvents.gen.js` and
`dist/v2/gen/core/serverSentEvents.gen.js` still handle a rejected
`reader.cancel()`. Keep `COPY --chown=node:node patches
/opt/bridge-build/patches` before the filtered `bun install` in
`docker/Dockerfile`: Bun validates root `patchedDependencies` even when the
filtered workspaces do not install OpenCode themselves.

`apps/backend/src/core/opencode-provider-dispose.test.ts` is the regression, and
it deliberately drives the real client against a real `Bun.serve` SSE endpoint:
the `eventHarness` fake in `agent-provider-test-support.ts` closes cleanly on
abort and cannot reproduce this, which is why the bug shipped.
