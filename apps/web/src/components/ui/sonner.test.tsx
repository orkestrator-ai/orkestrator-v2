...
describe("Toaster", () => {
  ...
  test("keeps toast identities stable across rapid exit-animation clicks", async () => {
    ...
    expect(mockToastDismiss.mock.calls.map(([id]) => id)).toEqual(["newest-toast", "middle-toast"]);
  ...
  test("supports Enter and Space without hijacking nested controls", async () => {
    ...
    expect(mockToastDismiss.mock.calls.map(([id]) => id)).toEqual(["enter-toast", "space-toast"]);
  ...
  test("scopes identical stacks to their owning toaster IDs", async () => {
    ...
    expect(mockToastDismiss.mock.calls.map(([id]) => id)).toEqual(["global-old", "canvas-new"]);
  });
