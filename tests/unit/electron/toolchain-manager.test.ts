...
async function tarGzip(
  entries: Array<{ name: string; body: Buffer; type?: "file" | "directory" }>,
): Promise<Buffer> {
  const pack = tar.pack();
...
describe("pinned desktop toolchain cache", () => {
  ...
  test("retains an extracted launcher bundle beside its runtime", async () => {
    ...
    await expect(
      readFile(path.join(path.dirname(target), "unpinned-runtime.js")),
  ...
  test("removes a fresh dead-owner lock but preserves a stale-looking live-owner lock", async () => {
    ...
    await writeFile(
      staleLock,
      JSON.stringify({ token: "dead", pid: 999_999, createdAt: new Date(0).toISOString() }),
    );
    ...
    await writeFile(
      liveLock,
      JSON.stringify({ token: "live", pid: process.pid, createdAt: new Date(0).toISOString() }),
    );
  ...
  test("detects a disappeared or replaced lock before releasing cache ownership", async () => {
    for (const replacement of [
      null,
      { token: "replacement", pid: 42, createdAt: new Date().toISOString() },
    ]) {
      const dataDir = await createDataDir();
      const lockPath = path.join(dataDir, "toolchains", ".install.lock");
      await expect(
        ...
        replacement ? "ownership changed unexpectedly" : "disappeared unexpectedly",
      );
    }
  });
  ...
  test("removes abandoned and failed staging directories while retaining no partial install", async () => {
    ...
    await expect(
      ensurePinnedToolchains({
        dataDir,
        artifacts: [
          { ...artifacts[0], archive: { ...artifacts[0].archive, sha256: "0".repeat(64) } },
        ],
        fetchImpl: createFetch(),
        skipExecutableProbeForTests: true,
      }),
  ...
  const downloadFailureCases: Array<{
    ...
    },
    {
      ...
      artifact: {
        ...artifacts[0],
        archive: { ...artifacts[0].archive, url: "http://downloads.example.test/codex.zip" },
      },
    ...
    },
    {
      ...
      artifact: {
        ...artifacts[0],
        archive: { ...artifacts[0].archive, url: "https://untrusted.example/codex.zip" },
      },
      fetchImpl: async () => response(ZIP_FIXTURE, { status: 200 }),
    ...
    },
    {
      ...
      fetchImpl: async () =>
        response(ZIP_FIXTURE, { status: 200, url: "https://untrusted.example/codex.zip" }),
      message: "redirected to an untrusted host",
    },
    {
      name: "mismatched content length",
      fetchImpl: async () =>
        response(ZIP_FIXTURE, { status: 200, headers: { "content-length": "1" } }),
    ...
    },
    {
      name: "oversized body",
      fetchImpl: async () =>
        response(Buffer.concat([ZIP_FIXTURE, Buffer.from([0])]), { status: 200 }),
  ...
  test("rejects malformed, unsafe, missing, duplicate, and mismatched archive entries", async () => {
    ...
    const cases = [
      {
        artifact: artifactWithBody(artifacts[0], Buffer.from("not a zip")),
        body: Buffer.from("not a zip"),
        message: "central directory",
      },
      {
        artifact: artifactWithBody(artifacts[0], unsafeZip),
        body: unsafeZip,
        message: "invalid relative path",
      },
      {
        artifact: { ...artifacts[0], archive: { ...artifacts[0].archive, entryPath: "missing" } },
        body: ZIP_FIXTURE,
      ...
      },
      {
        artifact: {
          ...artifacts[0],
          executable: { ...artifacts[0].executable, size: EXECUTABLE_SIZE - 1 },
        },
        body: ZIP_FIXTURE,
        message: "entry did not match",
      },
      {
        artifact: {
          ...artifacts[0],
          executable: { ...artifacts[0].executable, sha256: "0".repeat(64) },
        },
        body: ZIP_FIXTURE,
        message: "executable checksum",
      },
      {
        artifact: {
          ...artifacts[0],
      ...
      {
        artifact: artifactWithBody(artifacts[1], duplicateTar),
        body: duplicateTar,
        message: "duplicate executable entry",
      },
      { artifact: artifactWithBody(artifacts[1], nonTargetTar), body: nonTargetTar, message: "" },
      {
        artifact: artifactWithBody(artifacts[1], wrongTypeTar),
        body: wrongTypeTar,
        message: "entry did not match",
      },
      {
        artifact: artifactWithBody(artifacts[1], wrongSizeTar),
        body: wrongSizeTar,
        message: "entry did not match",
      },
      {
        artifact: artifactWithBody(artifacts[0], duplicateZip),
        body: duplicateZip,
        message: "duplicate executable entry",
      },
      {
        artifact: artifactWithBody(artifacts[0], invalidLocalHeaderZip),
        body: invalidLocalHeaderZip,
        message: "local file header",
      },
    ];
  ...
  test("reports deterministic macOS code-signature spawn, timeout, and nonzero failures", async () => {
    const cases: Array<{ outcome: SpawnOutcome; message: string; timeout?: number }> = [
      {
        outcome: { type: "error", error: new Error("codesign spawn failed") },
        message: "codesign spawn failed",
      },
  ...
  darwinTest(
    "repairs an invalid macOS signature from a manifest-pinned pristine copy",
    async () => {
      const dataDir = await createDataDir();
      const fetchImpl = createFetch();
      ...
      await writeFile(
        path.join(path.dirname(installedPath), ".installed.json"),
        JSON.stringify({
          size: 11,
  ...
  test("cleans repair temporaries after codesign I/O, timeout, and signing failures", async () => {
    const failures: Array<{ outcomes: SpawnOutcome[]; message: string; timeout?: number }> = [
      ...
      {
        outcomes: [{ type: "exit", code: 1 }, { type: "timeout" }],
  ...
  test("a live app lease prevents another build from pruning its old toolchain", async () => {
    ...
    for (const artifact of artifacts) {
      ...
      await expect(
        lstat(path.join(dataDir, "toolchains", artifact.name, artifact.version)),
      ).resolves.toBeDefined();
      expect(await readlink(path.join(olderResult.binDir, artifact.name))).toContain("/1.2.2/");
      expect(await readlink(path.join(currentResult.binDir, artifact.name))).toContain(
        `/${artifact.version}/`,
      );
    }
  ...
  test("repairs only the companions that are missing and leaves no staging behind", async () => {
    ...
    expect(
      (await readdir(path.join(dataDir, "toolchains"))).filter((entry) =>
  ...
  test("reports one progress stream covering the companion archives too", async () => {
    ...
    expect(events.slice(firstVerifying).some((event) => event.phase === "downloading")).toBe(false);
  ...
  test("rejects companion archives whose contents do not match the pinned manifest", async () => {
    ...
    const reject = async (artifact: ToolchainArtifact, archiveBody: Buffer, message: string) => {
      const dataDir = await createDataDir();
      const fetchImpl = mock(async (input: string) => {
  ...
  test("verifies a companion's macOS code signature without probing it", async () => {
    ...
    expect((await lstat(await readlink(path.join(result.binDir, "codex-host")))).size).toBe(
      companionBody.byteLength,
    );
  });
  ...
  test("rejects a companion whose name collides with another tool's activation link", async () => {
    ...
    const companionArchive = await tarGzip([{ name: "tool-host", body: companionBody }]);
    const fetchImpl = createCompanionFetch(companionArchive);
    const hijacksClaude = companionArtifact(artifacts[0], companionArchive, companionBody, {
      fileName: "claude",
    });
    ...
    for (const set of [
      [hijacksClaude, artifacts[1]],
      [artifacts[1], hijacksClaude],
    ]) {
      await expect(
        ensurePinnedToolchains({
          dataDir: await createDataDir(),
        ...
        "codex companion claude collides with claude in the shared activation directory",
      );
    }
    ...
    await expect(
      ensurePinnedToolchains({
        ...
        artifacts: [
          companionArtifact(artifacts[0], companionArchive, companionBody, {
            fileName: "shared-host",
          }),
          companionArtifact(artifacts[1], companionArchive, companionBody, {
            fileName: "shared-host",
          }),
        ],
        fetchImpl,
      ...
      "companion shared-host collides with codex in the shared activation directory",
    );
  ...
  test("rejects unsafe or colliding activation aliases before downloading", async () => {
    ...
    for (const set of [
      [aliasesClaude, artifacts[1]],
      [artifacts[1], aliasesClaude],
    ]) {
      await expect(
        ensurePinnedToolchains({
          dataDir: await createDataDir(),
        ...
        "codex activation alias claude collides with claude in the shared activation directory",
      );
    }
    expect(fetchImpl).toHaveBeenCalledTimes(0);
  });
