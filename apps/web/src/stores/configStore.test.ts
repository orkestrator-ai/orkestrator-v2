...
describe("configStore create-environment preferences", () => {
  beforeEach(() => {
    useConfigStore.setState({
      config: structuredClone(useConfigStore.getInitialState().config),
      isLoading: false,
  ...
  test("updates the remembered agent selection without replacing repository settings", () => {
    ...
    expect(useConfigStore.getState().config.repositories["project-1"]).toEqual({
      defaultBranch: "develop",
      prBaseBranch: "release",
      lastEnvironmentType: "local",
