...
describe("MultiReviewDefaultsEditor Fast defaults", () => {
  ...
  test("says feature and ticket builds use the same reviewer count", () => {
    ...
    expect(
      screen.getByText(
        "Choose how many independent reviewers a Multi Review click, feature build, or ticket build starts and the model each one uses. Reviewer 1 and Reviewer 2 are shared with the Defaults page.",
      ),
    ).toBeTruthy();
  });
  ...
  test("labels and clears a repository override as inheritance while retaining parent Fast", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "multi-review-default-0 choose Normal",
      }),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "multi-review-default-0 inherit speed",
      }),
    );
    expect(onChange.mock.calls.at(-1)?.[0].actionDefaults?.review).toEqual({
      platform: "claude",
  ...
  test("writes Fast onto the selected reviewer only", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "multi-review-default-0 choose Fast",
      }),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "multi-review-default-1 choose Normal",
      }),
    );
  ...
  test("keeps the model a reviewer was following when it pins Fast", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "multi-review-default-1 choose Fast",
      }),
    );
