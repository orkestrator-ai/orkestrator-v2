...
describe("StorageService environment attachment privacy", () => {
  test("clearing attachments scrubs retained backups and restricts permissions", async () => {
    await withStorage(async (storage, dataDir) => {
      ...
      expect((await backupContents(dataDir)).some((contents) => contents.includes(secret))).toBe(
        true,
      );
      ...
      expect(await fs.readFile(path.join(dataDir, "environments.json"), "utf8")).not.toContain(
        secret,
      );
      for (const contents of await backupContents(dataDir)) {
        expect(contents).not.toContain(secret);
      }
      const entries = (await fs.readdir(dataDir)).filter(
        (entry) => entry === "environments.json" || entry.startsWith("environments.json.bak."),
      );
  ...
  test("retries backup cleanup after the environment primary was already removed", async () => {
    await withStorage(async (storage, dataDir) => {
      ...
      await expect(storage.removeEnvironment(environment.id)).rejects.toThrow(
