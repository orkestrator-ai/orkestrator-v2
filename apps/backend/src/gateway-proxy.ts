...
export class GatewayProxy extends GatewayHandlers {
    ...
    await new Promise<void>((resolve) => {
      ...
      const proxyRequest = http.request(
        ...
        (proxyResponse) => {
          ...
          const upstreamContentEncoding = headerValueToString(
            proxyResponse.headers["content-encoding"],
          );
          const upstreamIsIdentity =
            normalizeContentEncoding(upstreamContentEncoding) === "identity";
          ...
          if (browserPreview && proxyPrefix && previewContentKind) {
            ...
            if (Number.isFinite(contentLength) && contentLength > MAX_BROWSER_PREVIEW_BODY_BYTES) {
              ...
              fail(
                new Error(
                  `Browser preview response exceeded ${MAX_BROWSER_PREVIEW_BODY_BYTES} bytes`,
                ),
              );
              return;
            }
            ...
            bodyStream.on("data", (chunk: Buffer | string) => {
              ...
              if (
                ...
                abortBody(
                  new Error(
                    `Browser preview response exceeded ${MAX_BROWSER_PREVIEW_BODY_BYTES} decoded bytes`,
                  ),
                );
                return;
              }
            ...
            bodyStream.once("end", () => {
              ...
              if (rewritten.byteLength > MAX_BROWSER_PREVIEW_BODY_BYTES) {
                ...
                fail(
                  new Error(
                    `Browser preview response exceeded ${MAX_BROWSER_PREVIEW_BODY_BYTES} rewritten bytes`,
                  ),
                );
                return;
              }
              ...
              settleRewrittenProxyBodyResponse(
                ...
                responseHeaders,
                prepareCompressedBody(rewritten, contentType, compressionContext),
                () => settled,
                finish,
                fail,
              );
            ...
            proxyResponse.once("aborted", () =>
              abortBody(new Error("Browser preview response was aborted")),
            );
            return;
          }
          ...
          const bodylessMetadataCouldDescribeCodedBytes =
            canTransform &&
          ...
          if (releaseBufferReservation) {
            ...
            if (!sourceBuffer) {
              ...
              proxyResponse.destroy();
              fail(
                new Error(`Could not allocate ${contentLength!} bytes for the proxied response`),
              );
              return;
            }
            let bytes = 0;
            ...
            proxyResponse.on("data", (chunk: Buffer | string) => {
              ...
              const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
              if (
                !canAppendToProxySourceBuffer(bytes, buffer.byteLength, sourceBuffer.byteLength)
              ) {
                abortBody(
                  new Error(
            ...
            proxyResponse.once("end", () => {
              ...
              response.removeListener("close", releaseWhileBuffering);
              if (
                shouldAbandonBufferedProxyBody(reservationReleased, settled, response.destroyed)
              ) {
                releaseReservation();
            ...
            proxyResponse.once("error", (error) => abortBody(error));
            proxyResponse.once("aborted", () =>
              abortBody(new Error("Proxied response was aborted")),
            );
            return;
          }
  ...
  protected async serveStatic(
    ...
    const preferredEncodings =
      allowCompression && isCompressibleStaticContentType(contentType)
