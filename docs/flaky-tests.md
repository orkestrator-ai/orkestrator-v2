...
| Standalone backend shutdown and Tailscale Serve lifecycle entries | Tests combine real backend startup, child-tree shutdown, and sometimes a second startup. The startup loop could also remain blocked in `reader.read()` past its nominal deadline. | Add a kill-backed 20-second readiness deadline, a 30-second file budget, and a 60-second budget for the two-lifecycle Serve case. | The built standalone owner passed with two workers; backend typecheck and the complete aggregate suite passed. |
...
post-fix aggregate passed in 145.1 seconds; a final aggregate was then run after
