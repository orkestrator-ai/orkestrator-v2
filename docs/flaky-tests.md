...
## `Electron backend command registry` split files, rotating 5-second timeouts (`tests/unit/electron/commands-registry-terminal.test.ts`, `commands-registry-pr.test.ts`, `commands-registry-environments.test.ts`)

- **Status:** open
- **Date observed:** 2026-08-17
- **Original command:** `bun run test:logged -- --name root-tests -- bun test ./tests --parallel=4 --only-failures`, at `a820125c88eee8fbbb2818bfd617e8dacb2c0057` on `investigate-agent-view`.
- **Worker configuration:** Root group alone with four Bun workers; no other suite running against this clone. The host was heavily loaded — the same command and commit took 122.1 s on an earlier run and 376.2 s / 292.6 s on the failing runs.
- **Failure:** Three different cases timed out across two runs, each at Bun's default 5-second per-test budget:
  - `rejects malformed container status framing and invalid encoded sections` (`commands-registry-terminal.test.ts:1384`), 5,004.35 ms.
  - `treats empty, null, and non-boolean draft output as non-draft` (`commands-registry-pr.test.ts`), 5,002.76 ms.
  - `clears a stale failure only once the stop has actually committed` (`commands-registry-environments.test.ts`), 5,001.26 ms.
- **Suite counts:** Run one: 3,726 passed, 2 failed. Run two: 3,727 passed, 1 failed.
- **Isolated rerun:** `bun test tests/unit/electron/commands-registry-terminal.test.ts` failed once (63 passed, 1 failed, 38.6 s) and then passed twice consecutively (exit 0 both times), so the isolated owner does recover.
- **Not caused by the change under review:** the same isolated failure reproduced with the working tree stashed at the identical commit, and the change on that branch touches only `apps/web/src/components/` renderer files.
- **Frequency:** two of four consecutive root-group runs at the same commit, with a different victim case each time.
- **Related:** the 2026-08-16 resolution-sweep row "Command-registry Git fixture, deduplicated/admitted container starts, and process-launch coverage", which raised the shared condition deadline to 10 s and gave the Git and launcher cases explicit budgets. These three cases were not among those given a budget: `commands-registry-terminal.test.ts` (62 tests), `commands-registry-pr.test.ts` (51) and `commands-registry-environments.test.ts` (114) declare no per-test `timeout:` at all, so every case in them still races real `withFakeDocker` shell spawns against Bun's 5-second default.
- **Hypothesis:** Each case spawns real `/bin/sh` fake-`docker` children — the framing case runs four `withFakeDocker` iterations, each executing `docker exec … bash -lc` over a large script — and 5 s is simply too tight for that under four-worker contention on a loaded host. The rotating victim and the ~5,001-5,004 ms durations both point at the outer budget rather than at any one assertion. Before raising a budget, a recurrence should measure how long a single `withFakeDocker` round trip actually takes on the failing host, so a genuine regression in the shared docker-exec path is not hidden by a larger number.
