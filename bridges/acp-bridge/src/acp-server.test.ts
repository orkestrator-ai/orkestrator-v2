...
describe("ACP bridge", () => {
  ...
  test("closes an outstanding Cursor replay process when the bridge shuts down", async () => {
    ...
    const startedPids = new Set(
      (stopped.match(/^start:(\d+)$/gm) ?? []).map((line) => line.slice("start:".length)),
    );
    const stoppedPids = new Set(
      (stopped.match(/^stop:(\d+)$/gm) ?? []).map((line) => line.slice("stop:".length)),
    );
  ...
  test("times out hung initialization and rejects malformed agent output", async () => {
    const hung = await spawnBridge({
      env: { FAKE_ACP_HANG_INITIALIZE: "1", ACP_RPC_TIMEOUT_MS: "30" },
    });
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const response = await nativeFetch(`${hung.base}/session/create`, {
        method: "POST",
        headers: hung.headers,
      });
    ...
    const response = await nativeFetch(`${malformed.base}/session/create`, {
      method: "POST",
  ...
  test("exits when its advertised parent dies", async () => {
    ...
    const exited = new Promise<void>((resolvePromise) =>
      bridge.child.once("exit", () => resolvePromise()),
    );
    parent.kill("SIGKILL");
    await Promise.race([
      exited,
      Bun.sleep(2_000).then(() => {
        throw new Error("ACP bridge did not exit after parent death");
      }),
    ]);
  ...
  test("keeps serving other sessions after one agent dies mid-turn", async () => {
    ...
    const [dying, healthy] = await Promise.all([
      nativeFetch(`${base}/session/create`, { method: "POST", headers }).then((response) =>
        response.json(),
    ...
    const failed = await waitFor(
      async () =>
        nativeFetch(`${base}/session/${dying.id}/status`, { headers }).then((response) =>
          response.json(),
        ) as Promise<{ status: string; error?: string }>,
      (value) => value.status === "error",
    );
    ...
    expect(
      (await nativeFetch(`${base}/session/${dying.id}/cancel`, { method: "POST", headers })).status,
    ).toBe(202);
    ...
    const session = await waitFor(
      async () =>
        nativeFetch(`${base}/session/${healthy.id}`, { headers }).then((response) =>
          response.json(),
