...
describe("ActionBar grid presentation", () => {
  ...
  test("shows the shortcut hints that match the native, terminal, and review controls", async () => {
    render(<ActionBar />);

    const cases = [
      ["New native agent tab", "⌘T · Right-click for options"],
      ["New terminal tab", "⌘R · Right-click for agent CLIs"],
      ["Code review", "⌘⇧R or right-click/long-press to configure"],
    ] as const;

    for (const [accessibleName, hint] of cases) {
      const button = screen.getByRole("button", { name: accessibleName });
      fireEvent.focus(button);
      expect(screen.getByText(hint)).toBeTruthy();
      fireEvent.blur(button);
      await waitFor(() => expect(screen.queryByText(hint) === null).toBe(true));
    }
  });
...
describe("ActionBar toolbar interactions", () => {
  ...
  test("leaves Command+N unhandled for the desktop or browser new-window accelerator", () => {
    ...
    const event = createEvent.keyDown(window, { key: "n", code: "KeyN", metaKey: true });
  ...
  test("maps Command+R exclusively to a plain terminal tab", () => {
    render(<ActionBar />);

    const event = createEvent.keyDown(window, { key: "r", code: "KeyR", metaKey: true });
    fireEvent(window, event);

    expect(event.defaultPrevented).toBe(true);
    expect(createTabMock.mock.calls).toEqual([["plain"]]);
    expect(launchNativeAgentJobMock).not.toHaveBeenCalled();
    expect(startLoopedReviewMock).not.toHaveBeenCalled();
    expect(screen.queryByRole("heading", { name: "Configure code review" }) === null).toBe(true);
  });
...
describe("ActionBar pull request actions", () => {
  test.each([
    ...
    "shows $name CI checks inside the View PR action",
    ({ summary, label, colorClass, liveText, state }) => {
      ...
      const viewPrLabel = screen.getByText("View PR");
      expect(status.parentElement).toBe(viewPrLabel.parentElement);
      expect(viewPrLabel.classList.contains(colorClass)).toBe(false);
    },
  );
  ...
  test("announces when the final pending check completes unsuccessfully", () => {
    ...
    const view = render(<ActionBar />);
    const runningLabel = screen.getByRole("status").getAttribute("aria-label");
    ...
    expect(completed.textContent).toBe("(3/4)");
    expect(completed.getAttribute("aria-label")).not.toBe(runningLabel);
  });

  test("keeps a known failure yellow while other CI checks are still running", () => {
    currentEnvironment = { ...selectedEnvironment, prState: "open" };
    ...
    expect(status.classList.contains("text-yellow-600")).toBe(true);
  });
