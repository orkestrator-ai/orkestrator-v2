...
describe("review workflow contract", () => {
  test("includes the fail-closed interactive request failure kind", () => {
    ...
    expect(new Set(REVIEW_WORKFLOW_FAILURE_KINDS).size).toBe(REVIEW_WORKFLOW_FAILURE_KINDS.length);
  });
  test("resolves the default and target-branch token", () => {
    ...
    expect(
      resolveReviewInstruction("main", "Compare {{targetBranch}} twice: {{targetBranch}}"),
  ...
  test("rejects prompt-unsafe target branches and oversized start fields", () => {
    ...
    expect(
      isStartLoopedReviewInput({ ...input, model: "m".repeat(LOOPED_REVIEW_MAX_MODEL_LENGTH) }),
    ).toBe(true);
    expect(
      isStartLoopedReviewInput({ ...input, model: "m".repeat(LOOPED_REVIEW_MAX_MODEL_LENGTH + 1) }),
    ).toBe(false);
    expect(
      isStartLoopedReviewInput({
        ...input,
  ...
  test("adopts legacy state only at explicit safe boundaries", () => {
    ...
    for (const phase of ["completed", "cancelled"]) {
      expect(isSafelyAdoptableLegacyLoopedReview({ version: 1, phase, dispatch: {} })).toBe(true);
    }
  ...
  test("validates complete nested backend snapshots and cross-field references", () => {
    ...
    const session = {
      ...
      round: 1,
      sessionKey: "workflow:preparation:1",
    ...
    expect(
      isLoopedReviewWorkflow({ ...valid, activePool: { issues: [{}], coverageGaps: [] } }),
    ).toBe(false);
  ...
  test("requires pause/cancellation resume metadata only in their owning phase", () => {
    ...
    expect(isLoopedReviewWorkflow(workflowFixture({ cancellingFromPhase: "discovering" }))).toBe(
      false,
    );
    expect(
      isLoopedReviewWorkflow(
...
const issueFixture = {
  ...
  line: 4,
  symbol: "advance",
  description: "The phase advances twice.",
  evidence: "Two callers pass the guard.",
  suggestion: "Persist a lease.",
  verification: "Reconnect mid-dispatch.",
};
...
describe("review package validation", () => {
  const withPackage = (overrides: Record<string, unknown>) =>
    workflowFixture({
      rounds: [
        {
          round: 1,
  ...
  test("accepts an absent or null context but rejects a malformed one", () => {
    ...
    expect(
      isLoopedReviewWorkflow(
        withPackage({
          context: {
            comments: Array.from({ length: LOOPED_REVIEW_MAX_CONTEXT_LIST_ENTRIES + 1 }, () => "c"),
          },
        }),
      ),
    ).toBe(false);
  ...
  test("validates changed-file and validation entries", () => {
    ...
    expect(isLoopedReviewWorkflow(withPackage({ changedFiles: [{ ...file, path: "" }] }))).toBe(
      false,
    );
    expect(isLoopedReviewWorkflow(withPackage({ changedFiles: [{ ...file, status: 3 }] }))).toBe(
      false,
    );
    ...
    expect(
      isLoopedReviewWorkflow(withPackage({ validation: [{ ...command, status: "errored" }] })),
    ).toBe(false);
    expect(
      isLoopedReviewWorkflow(withPackage({ validation: [{ ...command, exitCode: 1.5 }] })),
    ).toBe(false);
    expect(
      isLoopedReviewWorkflow(withPackage({ validation: [{ ...command, durationMs: -1 }] })),
    ).toBe(false);
    expect(isLoopedReviewWorkflow(withPackage({ validation: [{ ...command, command: "" }] }))).toBe(
      false,
    );
  });
  ...
  test("validates skipped and uncommitted file notes", () => {
    ...
    expect(isLoopedReviewWorkflow(withPackage({ skippedFiles: [{ path: "a", reason: "" }] }))).toBe(
      false,
    );
...
describe("reconciliation and finding-outcome validation", () => {
  const reconciliation = (overrides: Record<string, unknown>) =>
    workflowWithPass({
      ...
      reconciliation: {
        newIssues: [issueFixture],
        issueUpdates: [],
        newCoverageGaps: [gapFixture],
        coverageGapUpdates: [],
  ...
  test("validates the findings themselves, not merely that they are arrays", () => {
    ...
    expect(isLoopedReviewWorkflow(reconciliation({ newIssues: [{ title: "partial" }] }))).toBe(
      false,
    );
  ...
  test("ties each outcome to whether it has a pool entry yet", () => {
    ...
    expect(
      isLoopedReviewWorkflow(
        reconciliation({
          newIssues: [],
          issueUpdates: [{ poolId: "issue-1", finding: issueFixture }],
    ...
    expect(
      isLoopedReviewWorkflow(
        reconciliation({
          newIssues: [],
          issueOutcomes: [{ reportIndex: 0, outcome: "existing", poolId: "issue-1" }],
        }),
      ),
    ).toBe(true);
    expect(
...
describe("session, transcript and interaction bounds", () => {
  ...
  test("bounds provider-supplied interaction transcript text and collections", () => {
    const entry = {
      ...
      title: "Proceed?",
      questions: [{ prompt: "Which?", options: ["a", "b"] }],
    };
    ...
    expect(
      isLoopedReviewWorkflow(
        withSession({
          interactionTranscript: Array.from({ length: 65 }, (_, index) => ({
            ...entry,
            id: `i-${index}`,
          })),
        }),
  ...
  test("validates a pending interaction resolution", () => {
    ...
    expect(
      isLoopedReviewWorkflow(
        workflowFixture({
          ...base,
          pendingInteractionResolution: { ...pending, action: "approve" },
        } as unknown as Partial<LoopedReviewWorkflow>),
      ),
    ).toBe(false);
    expect(
      isLoopedReviewWorkflow(
        workflowFixture({
          ...base,
          pendingInteractionResolution: { ...pending, kind: "unknown-kind" },
        } as unknown as Partial<LoopedReviewWorkflow>),
      ),
    ).toBe(false);
    expect(
      isLoopedReviewWorkflow(
        workflowFixture({
          ...base,
          pendingInteractionResolution: { ...pending, title: "x".repeat(16_385) },
        } as unknown as Partial<LoopedReviewWorkflow>),
      ),
    ).toBe(false);
  });
...
describe("failure, dispatch, wait and pr blocks", () => {
  ...
  test("keeps the failure's interaction context content-free and well formed", () => {
    ...
    expect(
      isLoopedReviewWorkflow(failed({ interaction: { ...interaction, provider: "gemini" } })),
    ).toBe(false);
    expect(
      isLoopedReviewWorkflow(failed({ interaction: { ...interaction, kind: "unknown" } })),
    ).toBe(false);
    expect(isLoopedReviewWorkflow(failed({ interaction: { ...interaction, requestId: "" } }))).toBe(
      false,
    );
  });
  ...
  test("validates the dispatch state enum, including the at-most-once marker", () => {
    ...
    const withDispatch = (state: string) =>
      workflowFixture({
        phase: "discovering",
        sessions: [sessionFixture],
  ...
  test("validates the structured wait and its dispatch link", () => {
    ...
    const withWait = (wait: Record<string, unknown>) =>
      workflowFixture({
        phase: "discovering",
        sessions: [sessionFixture],
        dispatch,
...
describe("workflow-level structure and bounds", () => {
  test("rejects non-object and wrong-version inputs", () => {
    ...
    expect(isLoopedReviewWorkflow(workflowFixture({ controller: "renderer" } as never))).toBe(
      false,
    );
...
describe("target branch and allowance boundaries", () => {
  ...
  test("detects issues as well as coverage gaps", () => {
    expect(
      hasReviewFindings({ issues: [{ poolId: "issue-1", ...issueFixture }], coverageGaps: [] }),
    ).toBe(true);
    expect(
      hasReviewFindings({ issues: [], coverageGaps: [{ poolId: "gap-1", ...gapFixture }] }),
    ).toBe(true);
    expect(hasReviewFindings({ issues: [], coverageGaps: [] })).toBe(false);
  });
});
...
describe("review body assembly", () => {
  ...
  test("puts uncommitted work inside the automated review scope", () => {
    ...
    expect(body).toContain("`git diff origin/main...HEAD` covers only the committed part");
  ...
  test("commit mode does not contradict its own leave-uncommitted instruction", () => {
    ...
    expect(body).toContain("If a file looks suspicious or unrelated, leave it uncommitted");
    expect(body).toContain(
