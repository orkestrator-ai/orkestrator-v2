...
const downloaders: Downloader[] = [
  {
    ...
    expectedDownload: ({ architecture, os }) => {
      ...
      return `claude-code-${platform}-${arch}/-/claude-code-${platform}-${arch}-2.1.219.tgz`;
    },
  ...
  },
  {
    ...
    expectedDownload: ({ architecture, os }) => {
      ...
      return `rust-v0.145.0/codex-${target}.tar.gz`;
    },
    expectedExtractor: () => "tar",
  },
  {
    ...
    expectedDownload: ({ architecture, os }) => {
      ...
      return `v1.18.4/opencode-${platform}-${arch}${extension}`;
    },
