...
describe("Electron backend path safety", () => {
  ...
  test("reads a regular file through the bounded host-file helper", async () => {
    ...
    try {
      await expect(readReadableHostFile(filePath, [root])).resolves.toEqual(Buffer.from([0, 1, 2]));
    } finally {
