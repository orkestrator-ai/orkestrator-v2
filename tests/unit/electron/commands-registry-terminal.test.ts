...
`;
  ...
  test(
    "retains an exited terminal snapshot long enough for desync recovery",
    async () => {
      ...
      expect(await commands.get("get_terminal_output_snapshot")?.({ sessionId }, context)).toEqual(
        beforeExit,
      );
  ...
  test(
    "returns terminal deltas and falls back across output generations",
    async () => {
      ...
      expect(commands.get("terminal_write")?.({ sessionId, data: "x" }, context)).toEqual({
        delivered: true,
      });
      expect(commands.get("terminal_resize")?.({ sessionId, cols: 80, rows: 24 }, context)).toEqual(
        { delivered: true },
      );
      expect(
        commands.get("terminal_write")?.({ sessionId: "missing-session", data: "x" }, context),
      ).toEqual({ delivered: false });
      expect(
        commands.get("terminal_resize")?.(
          { sessionId: "missing-session", cols: 80, rows: 24 },
          context,
        ),
      ).toEqual({ delivered: false });
      expect(
        commands.get("local_terminal_write")?.(
          { sessionId: "missing-session", data: "x" },
          context,
        ),
      ).toEqual({ delivered: false });
      expect(
        commands.get("local_terminal_resize")?.(
          {
  ...
  test(
    "expires an exited terminal snapshot through the production timer path",
    async () => {
  ...
  test(
    "an explicitly closed terminal does not retain its output snapshot",
    async () => {
  ...
  test(
    "bounds retained exited terminal snapshots and evicts the oldest",
    async () => {
      ...
      expect(
        await commands.get("get_terminal_output_snapshot")?.({ sessionId: sessionIds[0] }, context),
  ...
  test(
    "caps the terminal output buffer at the maximum size",
    async () => {
      ...
      expect(commandTesting.terminalOutputBufferStats(sessionId).chunks).toBeLessThanOrEqual(1_024);
      const buffer = (await commands.get("get_terminal_output_buffer")?.(
        { sessionId },
        context,
      )) as string;
  ...
  test(
    "does not split a Unicode surrogate pair at the terminal transcript boundary",
    async () => {
      ...
      const snapshot = commands.get("get_terminal_output_snapshot")?.({ sessionId }, context) as {
        output: string;
        truncated: boolean;
      };
  ...
  test(
    "does not leave a low surrogate when the trimmed pair spans PTY chunks",
    async () => {
  ...
  test("can limit local git stats to committed changes since environment creation", async () => {
    ...
    await expect(
      commands.get("get_local_git_status")?.(
        {
          worktreePath: worktree,
          targetBranch: creationCommit,
          includeUncommitted: false,
  ...
  test("reverts tracked and newly added local files to the target branch", async () => {
    ...
    await expect(
      commands.get("revert_local_file")?.(
  ...
  test("reverts both endpoints of a local rename", async () => {
    ...
    await expect(fs.readFile(path.join(worktree, "original.txt"), "utf8")).resolves.toBe(
      "original\n",
    );
  ...
  test("deletes local files and stages tracked deletions for the next commit", async () => {
    ...
    await expect(
      commands.get("delete_local_file")?.(
  ...
  test("does not delete a local file when the revert target ref is missing", async () => {
    ...
    await expect(fs.readFile(path.join(worktree, "tracked.txt"), "utf8")).resolves.toBe(
      "changed\n",
    );
  });
  ...
  test("counts zero added lines for empty and binary untracked files", async () => {
    ...
    expect(changes).toContainEqual(
      expect.objectContaining({ path: "empty.txt", additions: 0, status: "?" }),
    );
    expect(changes).toContainEqual(
      expect.objectContaining({ path: "binary.bin", additions: 0, status: "?" }),
    );
  });
...
`,
  ...
  test("injects workspace artifact git excludes before reading container git status", async () => {
    ...
    const { context } = createContext(environment);
    const commands = createCommandRegistry();
    const framedStatus = framedContainerGitStatus("M\0tracked.txt\0", "1\t2\ttracked.txt\0");
...
`,
  ...
  test("uses HEAD and omits worktree scanning for committed-only container status", async () => {
    ...
    const commands = createCommandRegistry();
    const framedStatus = framedContainerGitStatus("M\0committed.txt\0", "1\t0\tcommitted.txt\0");
...
`,
  ...
  test("does not confuse marker-like text in a container path with response framing", async () => {
    ...
    const framedStatus = framedContainerGitStatus(`M\0${markerPath}\0`, `3\t1\t${markerPath}\0`);
...
`,
      async (logs) => {
        await expect(
          commands.get("revert_container_file")?.(
  ...
  liveDockerTest(
    "executes rename-aware and containment-safe file mutations in a live container",
    async () => {
  ...
  test("does not consume terminal bootstrap ownership when delivery is unavailable or throws", async () => {
    ...
    expect(
      await commands.get("bootstrap_terminal_session")?.(
    ...
    expect(() =>
      commands.get("bootstrap_terminal_session")?.({ sessionId, data: "codex\n" }, context),
  ...
  test("reattaches a stable terminal tab to the same backend PTY and buffer", async () => {
    ...
    const firstResult = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(args, context),
    );
    const firstSessionId = firstResult.sessionId;
    expect(firstResult.created).toBe(true);
    await commands.get("start_local_terminal_session")?.({ sessionId: firstSessionId }, context);
    ...
    const reattachedResult = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(args, context),
    );
    ...
    expect(
      commands.get("get_terminal_output_snapshot")?.({ sessionId: reattachedSessionId }, context),
    ).toEqual({
    ...
    expect(emitted.filter(({ event }) => event === `terminal-output-${firstSessionId}`)).toEqual([
      {
    ...
    ptyProcesses[0]?.emitExit({ exitCode: 0 });
    const exitedResult = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(args, context),
    );
    expect(exitedResult).toEqual({
      sessionId: firstSessionId,
      created: false,
      bootstrapped: false,
    });
    expect(
      commands.get("get_terminal_output_snapshot")?.({ sessionId: firstSessionId }, context),
    ).toEqual({
      output: "dev server listening on 3000\r\nsecond chunk\r\n",
      revision: 2,
      generation: 1,
    ...
    await commands.get("close_local_terminal_session")?.({ sessionId: firstSessionId }, context);
    const replacementResult = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(args, context),
    );
  ...
  test("supersedes a stable local terminal when its activity configuration changes", async () => {
    ...
    const untracked = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(baseArgs, context),
    );
    await commands.get("start_local_terminal_session")?.(
    ...
    const tracked = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(
    ...
    const reused = terminalSessionResult(
      await commands.get("create_local_terminal_session")?.(
        { ...baseArgs, trackEnvironmentActivity: true },
        context,
      ),
  ...
  test(
    "records prompt and settled-output activity for tracked local agent terminals",
    async () => {
      ...
      expect(recordActivity).toHaveBeenLastCalledWith(environment.id, "2026-07-23T10:00:00.000Z");
      ...
      expect(recordCompletion).toHaveBeenLastCalledWith(environment.id, "2026-07-23T10:05:00.000Z");
        ...
        () =>
          emitted.some(
            ({ event, payload }) =>
              ...
              (payload as { environment_id?: string; occurred_at?: string; activity_kind?: string })
                .environment_id === environment.id &&
              (payload as { environment_id?: string; occurred_at?: string; activity_kind?: string })
                .occurred_at === "2026-07-23T10:05:00.000Z" &&
              (payload as { environment_id?: string; occurred_at?: string; activity_kind?: string })
  ...
  test(
    "retries tracked terminal completion persistence and notification before consuming the edge",
    async () => {
  ...
  test(
    "deduplicates settled output per prompt generation and admits the next prompt",
    async () => {
      ...
      const notifyAgentTurnCompleted = mock(() =>
        notifyAgentTurnCompleted.mock.calls.length === 1 ? firstNotification : Promise.resolve(),
      );
      context.notifyAgentTurnCompleted = notifyAgentTurnCompleted;
      const recordCompletion = context.storage.recordEnvironmentCompletion as ReturnType<
        typeof mock
      ...
      await waitForCondition(
        () => notifyAgentTurnCompleted.mock.calls.length === 2,
        "second generation notification",
      );
  ...
  test.each([
    ...
    "consumes a tracked terminal edge after %s retry exhaustion",
    async (_label, failPersistence) => {
  ...
  test(
    "does not carry a completed turn onto a replacement PTY with the same stable key",
    async () => {
      ...
      const first = terminalSessionResult(
        await commands.get("create_local_terminal_session")?.(args, context),
      );
      await commands.get("start_local_terminal_session")?.({ sessionId: first.sessionId }, context);
      await commands.get("local_terminal_write")?.(
        { sessionId: first.sessionId, data: "codex\r" },
        context,
      );
      ptyProcesses[0]?.emitExit({ exitCode: 0 });
      await waitForCondition(
      ...
      const replacement = terminalSessionResult(
        await commands.get("create_local_terminal_session")?.(args, context),
      );
  ...
  test(
    "debounces repeated output and ignores writes without a submitted prompt",
    async () => {
  ...
  test(
    "logs terminal activity persistence failures without emitting a success event",
    async () => {
  ...
  test(
    "records prompt and settled-output activity for tracked container agent terminals",
    async () => {
      const environment = createEnvironment({
      ...
      expect(recordActivity).toHaveBeenCalledWith(environment.id, "2026-07-23T11:00:00.000Z");
      expect(recordCompletion).toHaveBeenCalledWith(environment.id, "2026-07-23T11:02:00.000Z");
      await waitForCondition(
        () => notifyAgentTurnCompleted.mock.calls.length === 1,
  ...
  test("rejects local terminal start when the worktree path is missing", async () => {
    ...
    await expect(
      commands.get("start_local_terminal_session")?.({ sessionId }, context),
  ...
  test("starts legacy terminal session identifiers without a remembered config", async () => {
    ...
    await commands.get("start_local_terminal_session")?.({ sessionId: localSessionId }, context);
    await commands.get("start_terminal_session")?.({ sessionId: containerSessionId }, context);
  ...
  test("starts container terminal sessions through docker exec in a PTY", async () => {
    ...
    const firstResult = terminalSessionResult(
      await commands.get("create_terminal_session")?.(createArgs, context),
    );
    ...
    const reattachedSessionId = reattachedResult.sessionId;
    expect(reattachedResult.created).toBe(false);
    await commands.get("start_terminal_session")?.({ sessionId: reattachedSessionId }, context);
  ...
  test("supersedes a stable container terminal when its target or activity context changes", async () => {
    ...
    const changedContainer = terminalSessionResult(
      await commands.get("create_terminal_session")?.(
    ...
    const changedUser = terminalSessionResult(
      await commands.get("create_terminal_session")?.(
    ...
    const changedActivity = terminalSessionResult(
      await commands.get("create_terminal_session")?.(
        {
          ...baseArgs,
  ...
  test(
    "does not start a container terminal after its stable session is superseded during lookup",
    async () => {
      const environment = createEnvironment({
      ...
      const first = terminalSessionResult(
        await commands.get("create_terminal_session")?.({ ...baseArgs, user: "node" }, context),
      );
      ...
      const replacement = terminalSessionResult(
        await commands.get("create_terminal_session")?.({ ...baseArgs, user: "root" }, context),
      );
      releaseLookup();

      await expect(startPromise).rejects.toThrow(
        "Container terminal session is no longer available",
      );
