...
describe("parseSlashCommands", () => {
  ...
  test("adds the leading slash when the SDK omits it", () => {
    expect(parseSlashCommands(["compact"])).toEqual([{ name: "/compact", description: undefined }]);
  ...
  test("keeps commands whose names merely share a prefix", () => {
    ...
    expect(commands.map((command) => command.name)).toEqual(["/test", "/testing"]);
  });
  ...
  test("sorts by name so the menu order does not depend on the server", () => {
    ...
    expect(commands.map((command) => command.name)).toEqual(["/alpha", "/mid", "/zeta"]);
  });

  test("tolerates degenerate entries rather than throwing", () => {
    expect(parseSlashCommands(["/"])).toEqual([{ name: "/", description: undefined }]);
    expect(parseSlashCommands([""])).toEqual([{ name: "/", description: undefined }]);
  });
});
