...
4. Each command has a unique short id, a non-interactive shell command, a workspace-relative cwd (usually "."), and dependsOn listing prerequisite ids EARLIER in the array. Split independent work so it can run concurrently. Inspect what each selected stage actually runs before adding another command: if the full test stage already runs the production build, omit a separate build command instead of repeating it. Shared build prerequisites run once; avoid overlapping aggregate commands that repeat the same validation. Preserve necessary build/test dependencies and setup/cleanup semantics. Prefer a repository-owned one-shot lifecycle task for service-backed checks. Never place a foreground server/supervisor before its dependent tests in a sequential shell command: readiness output does not mean that the starter exits. Only when no repository-owned lifecycle task exists may tightly coupled setup, test, and cleanup use a shell trap, with explicit readiness, bounded waits, supervision, and cleanup.
5. resources names directories or shared services the command writes or consumes exclusively (for example a generated output directory, test database, or simulator). Commands with the same resource serialize. Use workspace: prefixes for workspace directories and services (for example workspace:dist); use ["workspace:*"] when interference within this workspace is uncertain. Use host: prefixes for genuinely host-shared resources (for example host:tcp:1422 or host:ios-simulator), and ["host:*"] only when host-wide interference is uncertain. Legacy unprefixed resource names and ["*"] retain host-wide scope for compatibility: do not use them for workspace-only uncertainty. Empty resources means you have verified parallel safety. weight=2 reserves the runner for internally parallel or memory-heavy work; weight=1 allows two independent commands concurrently. Do not guess a command is lightweight. timeoutMs must be between 1000 and 7200000 and appropriate to this project. Never request watch mode, interactive input, background servers without cleanup, or a detached process. If .orkestrator-test-scheduler.json is present, its cooperativeCommands are eligible alternatives, NOT a checklist. Select only the required coverage, using each selected command verbatim from cwd="." without shell wrappers or compound commands; those runners reserve their own host capacity. Select repository-owned commandProfiles entries verbatim from cwd="." as separate commands too; do not synthesize setup/test/cleanup wrappers around an available one-shot task. Only cooperativeCommands reserve their own capacity; ordinary profiled commands are admitted by the review worker. Follow exact commandProfiles resource and coverage declarations when present. A profile covers list declares commands whose validation is already included: choose the covering command once instead of also scheduling its covered alternatives. Waiting for capacity does not count against timeoutMs.
