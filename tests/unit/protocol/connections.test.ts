...
describe("connection protocol validation", () => {
  ...
  test("rejects malformed stored records and malformed renderer summaries", () => {
    ...
    expect(() => parseStoredDesktopConnections({ activeConnectionId: "local" })).toThrow(
      "connections",
    );
    expect(() =>
      parseStoredDesktopConnections({ activeConnectionId: "local", connections: [{}] }),
    ).toThrow("connections[0].id");
    expect(() =>
      parseConnectionList({
