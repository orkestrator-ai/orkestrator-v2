...
export function isStructuredCommandError(value: unknown): value is StructuredCommandError {
...
export function isAwaitBridgeReadyResult(value: unknown): value is AwaitBridgeReadyResult {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const record = value as Record<string, unknown>;
